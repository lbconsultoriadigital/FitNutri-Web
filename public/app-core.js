...
const state={csrf:null,selectedId:null,poll:null,processing:false,jobs:[],selectedFile:null,currentJob:null};
...
function notify(message){const el=$('flash');el.textContent=message;el.classList.remove('hidden');clearTimeout(el._t);el._t=setTimeout(()=>el.classList.add('hidden'),4200)}
...
async function api(path,options={}){const headers=new Headers(options.headers||{});const method=(options.method||'GET').toUpperCase();if(options.body&&!(options.body instanceof FormData)&&!headers.has('Content-Type'))headers.set('Content-Type','application/json');if(state.csrf&&['POST','PUT','PATCH','DELETE'].includes(method))headers.set('X-CSRF-Token',state.csrf);const res=await fetch(path,{...options,headers,credentials:'same-origin'});const type=res.headers.get('content-type')||'';let data;try{data=type.includes('json')?await res.json():await res.text()}catch{data={}}if(!res.ok)throw new Error(data.detail||data||`HTTP ${res.status}`);return data}
